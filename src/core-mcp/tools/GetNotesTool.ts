...
export class GetNotesTool extends BaseTool {
  ...
  schema = z.object({
    ...
    filterTypes: z
      ...
      .optional()
      .describe(
        'Filter results to only these note types. Leave empty to include all types'
      ),
  ...
  async execute(input: z.infer<typeof this.schema>): Promise<McpToolResult> {
    ...
    if (parsed.filterTypes && parsed.filterTypes.length > 0) {
      filteredNotes = filteredNotes.filter(note =>
        parsed.filterTypes!.includes(note.type as any)
      );
    }
    ...
    filteredNotes.sort((a, b) => {
      switch (parsed.sortBy) {
        ...
        case 'relevance':
          ...
          const aIsExact = a.anchors.some(anchor => 
            path.join(repoRoot, anchor) === parsed.path
          );
          const bIsExact = b.anchors.some(anchor => 
            path.join(repoRoot, anchor) === parsed.path
          );
