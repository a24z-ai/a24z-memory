...
export class DeleteNoteTool extends BaseTool {
  ...
  schema = z.object({
    noteId: z.string().describe('The unique ID of the note to delete (e.g., "note-1734567890123-abc123def")'),
    directoryPath: z.string().describe('The absolute path to the git repository root directory or any path within it. The tool will find the repository root automatically.'),
  });
  ...
  async execute(input: z.input<typeof this.schema>): Promise<McpToolResult> {
    ...
    if (deleted) {
      ...
      throw new Error(`Failed to delete note ${parsed.noteId}. The note may have already been deleted.`);
    }
  }
}