...
describe('Repository-Specific Storage', () => {
  ...
  beforeEach(() => {
    // Disable the global test data directory override for these tests
    originalTestDataDir = process.env.A24Z_TEST_DATA_DIR;
  ...
  afterEach(() => {
    ...
    // Restore the original test data directory
    if (originalTestDataDir) {
      process.env.A24Z_TEST_DATA_DIR = originalTestDataDir;
    }
  });
  ...
  describe('Repository Isolation', () => {
    ...
    beforeEach(() => {
      // Ensure test data directory is still disabled
      delete process.env.A24Z_TEST_DATA_DIR;
